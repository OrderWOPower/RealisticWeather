...
namespace RealisticWeatherNaval
{
	...
	public class RealisticWeatherNavalMixin : BaseViewModelMixin<NavalCustomBattleMapSelectionGroupVM>
	{
		...
		public IEnumerable<(string, int)> FogDensities
		{
			get
			{
				yield return (new TextObject("{=RealisticWeather01}None").ToString(), 1);
				yield return (new TextObject("{=RealisticWeather02}Light").ToString(), 8);
				yield return (new TextObject("{=RealisticWeather03}Moderate").ToString(), 16);
				yield return (new TextObject("{=RealisticWeather04}Heavy").ToString(), 32);
				yield return (new TextObject("{=RealisticWeather05}Very Heavy").ToString(), 64);
		...
		public override void OnRefresh()
		{
			...
			foreach ((string, int) fogDensity in FogDensities)
			{
